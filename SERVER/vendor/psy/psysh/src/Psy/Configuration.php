...
class Configuration {
	...
	public function getConfigFile() {
		...
		$files = ConfigPaths::getConfigFiles ( array (
				'config.php',
				'rc.php' 
		), $this->configDir );
	...
	public function getHistoryFile() {
		...
		if (@is_file ( $oldHistory )) {
			...
			$msg = sprintf ( "PsySH history file found at '%s'. Please delete it or move it to '%s'.", strtr ( $oldHistory, '\\', '/' ), $newHistory );
	...
	public function getManualDbFile() {
		...
		$files = ConfigPaths::getDataFiles ( array (
				'php_manual.sqlite' 
		), $this->dataDir );
		if (! empty ( $files )) {
			if ($this->warnOnMultipleConfigs && count ( $files ) > 1) {
