...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	public function testMultiValuedFields() {
		...
		$this->assertEquals ( array_keys ( $form->all () ), array (
				'foo[2]',
				'foo[3]',
				'bar[foo][0]',
				'bar[foo][foobar]' 
	...
	public function testGetValues() {
		...
		$this->assertEquals ( array (
				'bar' => 'bar' 
		), $form->getValues (), '->getValues() does not include not-checked checkboxes' );
		
		$form = $this->createForm ( '<form><input type="file" name="foo" value="foo" /><input type="text" name="bar" value="bar" /><input type="submit" /></form>' );
		$this->assertEquals ( array (
				'bar' => 'bar' 
		), $form->getValues (), '->getValues() does not include file input fields' );
		...
		$this->assertEquals ( array (
				'bar' => 'bar' 
		), $form->getValues (), '->getValues() does not include disabled fields' );
	}
	...
	public function testMultiselectSetValues() {
		...
		), $form->getValues (), '->setValue() sets the values of select' );
	}
	public function testGetPhpValues() {
		...
		$this->assertEquals ( array (
				...
				'ba r' => 'bar' 
		), $form->getPhpValues (), '->getPhpValues() preserves periods and spaces in names' );
		...
		), $form->getPhpValues (), '->getPhpValues() preserves periods and spaces in names recursively' );
		...
		$this->assertEquals ( array (
				...
				'bar' => 'bar' 
		), $form->getPhpValues (), "->getPhpValues() doesn't return empty values" );
	}
	...
	protected function getFormFieldMock($name, $value = null) {
		$field = $this->getMockBuilder ( 'Symfony\\Component\\DomCrawler\\Field\\FormField' )->setMethods ( array (
				'getName',
				'getValue',
				'setValue',
				'initialize' 
	...
	public function testgetPhpValuesWithEmptyTextarea() {
		...
		$nodes = $dom->getElementsByTagName ( 'form' );
		$form = new Form ( $nodes->item ( 0 ), 'http://example.com' );
		$this->assertEquals ( $form->getPhpValues (), array (
				'example' => '' 
		) );
	}
}
