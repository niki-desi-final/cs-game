...
class RouteCollectionBuilderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testFlushSetsDetailsOnChildrenRoutes() {
		...
		setDefault ( 'page', 1 )->setRequirement ( 'id', '\d+' )->setOption ( 'expose', true )->
		// things that the collection will try to override (but won't)
		setDefault ( '_format', 'html' )->setRequirement ( '_format', 'json|xml' )->setOption ( 'fooBar', true )->setHost ( 'example.com' )->setCondition ( 'request.isSecure()' )->setSchemes ( array (
				'https' 
		) )->setMethods ( array (
				'POST' 
		) );
		...
		setDefault ( '_format', 'json' )->setRequirement ( '_format', 'xml' )->setOption ( 'fooBar', false )->setHost ( 'symfony.com' )->setCondition ( 'request.query.get("page")==1' )->
		// some unique things that should be set on the child
		setDefault ( '_locale', 'fr' )->setRequirement ( '_locale', 'fr|en' )->setOption ( 'niceRoute', true )->setSchemes ( array (
				'http' 
		) )->setMethods ( array (
				'GET',
				'POST' 
