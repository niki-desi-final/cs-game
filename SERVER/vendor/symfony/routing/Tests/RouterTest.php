...
class RouterTest extends \PHPUnit_Framework_TestCase {
	...
	public function testThatRouteCollectionIsLoaded() {
		...
		$this->loader->expects ( $this->once () )->method ( 'load' )->with ( 'routing.yml', 'ResourceType' )->will ( $this->returnValue ( $routeCollection ) );
	...
	public function testMatcherIsCreatedIfCacheIsNotConfigured($option) {
		...
		$this->loader->expects ( $this->once () )->method ( 'load' )->with ( 'routing.yml', null )->will ( $this->returnValue ( $this->getMock ( 'Symfony\Component\Routing\RouteCollection' ) ) );
