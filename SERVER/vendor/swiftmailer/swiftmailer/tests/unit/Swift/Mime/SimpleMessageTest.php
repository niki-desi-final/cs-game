...
class Swift_Mime_SimpleMessageTest extends Swift_Mime_MimePartTest {
	...
	public function testSenderIsSetInHeader() {
		...
		$sender->shouldReceive ( 'setFieldBodyModel' )->once ()->with ( array (
				'other@domain' => 'Other' 
		) );
	...
	public function testNameCanBeUsedInSenderHeader() {
		$headers = $this->_createHeaderSet ( array (), false );
		$headers->shouldReceive ( 'addMailboxHeader' )->once ()->with ( 'Sender', array (
				'sender@domain' => 'Name' 
		) );
		$headers->shouldReceive ( 'addMailboxHeader' )->zeroOrMoreTimes ();
	...
	public function testFromIsSetInHeader() {
		...
		$from->shouldReceive ( 'setFieldBodyModel' )->once ()->with ( array (
				'other@domain' => 'Other' 
		) );
	...
	public function testPersonalNameCanBeUsedInFromAddress() {
		$headers = $this->_createHeaderSet ( array (), false );
		$headers->shouldReceive ( 'addMailboxHeader' )->once ()->with ( 'From', array (
				'from@domain' => 'Name' 
		) );
		$headers->shouldReceive ( 'addMailboxHeader' )->zeroOrMoreTimes ();
	...
	public function testReplyToIsSetInHeader() {
		...
		$reply->shouldReceive ( 'setFieldBodyModel' )->once ()->with ( array (
				'other@domain' => 'Other' 
		) );
	...
	public function testReplyToIsAddedToHeadersDuringAddReplyTo() {
		...
		$replyTo->shouldReceive ( 'setFieldBodyModel' )->once ()->with ( array (
				'from@domain' => 'Name',
				'other@domain' => 'Other' 
		) );
	...
	public function testNameCanBeUsedInReplyTo() {
		$headers = $this->_createHeaderSet ( array (), false );
		$headers->shouldReceive ( 'addMailboxHeader' )->once ()->with ( 'Reply-To', array (
				'reply@domain' => 'Name' 
		) );
		$headers->shouldReceive ( 'addMailboxHeader' )->zeroOrMoreTimes ();
	...
	public function testToIsSetInHeader() {
		...
		$to->shouldReceive ( 'setFieldBodyModel' )->once ()->with ( array (
				'other@domain' => 'Other' 
		) );
	...
	public function testNameCanBeUsedInToHeader() {
		$headers = $this->_createHeaderSet ( array (), false );
		$headers->shouldReceive ( 'addMailboxHeader' )->once ()->with ( 'To', array (
				'to@domain' => 'Name' 
		) );
		$headers->shouldReceive ( 'addMailboxHeader' )->zeroOrMoreTimes ();
	...
	public function testCcIsSetInHeader() {
		...
		$cc->shouldReceive ( 'setFieldBodyModel' )->once ()->with ( array (
				'other@domain' => 'Other' 
		) );
	...
	public function testNameCanBeUsedInCcHeader() {
		$headers = $this->_createHeaderSet ( array (), false );
		$headers->shouldReceive ( 'addMailboxHeader' )->once ()->with ( 'Cc', array (
				'cc@domain' => 'Name' 
		) );
		$headers->shouldReceive ( 'addMailboxHeader' )->zeroOrMoreTimes ();
	...
	public function testBccIsSetInHeader() {
		...
		$bcc->shouldReceive ( 'setFieldBodyModel' )->once ()->with ( array (
				'other@domain' => 'Other' 
		) );
	...
	public function testNameCanBeUsedInBcc() {
		$headers = $this->_createHeaderSet ( array (), false );
		$headers->shouldReceive ( 'addMailboxHeader' )->once ()->with ( 'Bcc', array (
				'bcc@domain' => 'Name' 
		) );
		$headers->shouldReceive ( 'addMailboxHeader' )->zeroOrMoreTimes ();
	...
	public function testReadReceiptHeaderIsAddedIfNoneSet() {
		$headers = $this->_createHeaderSet ( array (), false );
		$headers->shouldReceive ( 'addMailboxHeader' )->once ()->with ( 'Disposition-Notification-To', 'mark@swiftmailer.org' );
		$headers->shouldReceive ( 'addMailboxHeader' )->zeroOrMoreTimes ();
	...
	public function testFluidInterface() {
		$child = $this->_createChild ();
		$message = $this->_createMessage ( $this->_createHeaderSet (), $this->_createEncoder (), $this->_createCache () );
		$this->assertSame ( $message, $message->setContentType ( 'text/plain' )->setEncoder ( $this->_createEncoder () )->setId ( 'foo@bar' )->setDescription ( 'my description' )->setMaxLineLength ( 998 )->setBody ( 'xx' )->setBoundary ( 'xyz' )->setChildren ( array () )->setCharset ( 'iso-8859-1' )->setFormat ( 'flowed' )->setDelSp ( false )->setSubject ( 'subj' )->setDate ( 123 )->setReturnPath ( 'foo@bar' )->setSender ( 'foo@bar' )->setFrom ( array (
				'x@y' => 'XY' 
		) )->setReplyTo ( array (
				'ab@cd' => 'ABCD' 
		) )->setTo ( array (
				'chris@site.tld',
				'mark@site.tld' 
		) )->setCc ( 'john@somewhere.tld' )->setBcc ( array (
				'one@site',
				'two@site' => 'Two' 
